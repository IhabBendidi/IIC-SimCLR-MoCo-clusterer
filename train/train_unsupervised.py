...
def trainGAN_UNSUP(data_loader, networks, opts, epoch, args, additional):
    ...
    for i in t_train:
        ...
        q_cont = C.moco_train(x_org)
        k_cont = C_EMA.moco_train(x_tf)

        k_cont = k_cont.detach()

        k2_cont = C_EMA.moco_train(x_tf_2)
        ...
        #over_iic_loss = calc_iic_loss(q_over_disc, k_over_disc)
        #iic_loss += 10 * over_iic_loss
        ...
        style_cont = C_EMA.moco(x_org)
        q_f = style_cont.detach().cpu().tolist()
